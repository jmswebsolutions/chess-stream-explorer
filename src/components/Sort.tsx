...
export const Sort: React.FC<SortProps> = ({ sortBy, onSortChange }) => {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 shadow-lg mb-6" role="region" aria-label="Sort options">
      <div className="flex items-center gap-2 mb-4">
        <FaSort className="text-blue-400" aria-hidden="true" />
        <h2 className="text-white font-semibold text-lg">{t('sort.title')}</h2>
      </div>

      <fieldset className="space-y-2 border-0 p-0 m-0">
        <legend className="sr-only">{t('sort.title')}</legend>
        <div role="radiogroup" aria-label="Sort by">
          <label className="flex items-center gap-2 text-gray-300 cursor-pointer hover:text-white transition-colors">
            <input
              ...
              aria-label={`${t('sort.username')} (A-Z)`}
            />
            {t('sort.username')} (A-Z)
          </label>
          ...
          <label className="flex items-center gap-2 text-gray-300 cursor-pointer hover:text-white transition-colors">
            <input
              ...
              className="w-4 h-4 bg-gray-700 border-gray-600 text-blue-500 focus:ring-blue-500"
              aria-label={`${t('sort.username')} (Z-A)`}
            />
            {t('sort.username')} (Z-A)
          </label>
          ...
          <label className="flex items-center gap-2 text-gray-300 cursor-pointer hover:text-white transition-colors">
            <input
              ...
              aria-label={`${t('sort.status')}: ${t('stats.live')}`}
            />
          ...
          <label className="flex items-center gap-2 text-gray-300 cursor-pointer hover:text-white transition-colors">
            <input
              ...
              aria-label={`${t('sort.status')}: ${t('stats.offline')}`}
            />
          ...
          <label className="flex items-center gap-2 text-gray-300 cursor-pointer hover:text-white transition-colors">
            ...
            />
            <FaEye className="text-purple-400 text-xs" aria-hidden="true" />
            Most Viewed
          </label>
